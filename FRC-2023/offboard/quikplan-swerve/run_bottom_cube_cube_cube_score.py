...
def plan(alliance, quiet):
    ...
    start_pose = pc.construct_pose(*BOTTOM_START_POS_PC)
    qp = QuikPlan(
        field,
        robot,
        start_pose,
        [StoppedPoseConstraint(start_pose)],
        start_action=Action(ActionType.POOP_CHUTE, scoring_loc=(2, 1)),
    )

    # Leave community without turning
    waypoint1 = pc.construct_pose(*BOTTOM_INTAKE_DEPLOY_PC)
    qp.add_waypoint(
        waypoint1,
        10,
    ...
    waypoint11 = pc.construct_pose(*BOTTOM_THIRD_SCORING_POS)
    qp.add_waypoint(
        waypoint11,
        10,
        intermediate_constraints=[
            AngularConstraint(waypoint11),  # Maintain heading within the community
            SpeedConstraint(SLOW_BUMP_SPEED),
        ],
        end_constraints=[
            StoppedPoseConstraint(waypoint11),  # We want to be stopped when scoring
        ],
        end_action=Action(ActionType.SCORE_PIECE, scoring_loc=(2, 4)),
    )
    ...
    write_to_csv(traj, f"{alliance}_bottom_cube_cube_cube_score")

    # Plot
    field.plot_traj(
        robot,
        traj,
        f"{alliance}_bottom_cube_cube_cube_score.png",
        save=True,
        quiet=quiet,
    )
    ...
    if not quiet:
        # Animate
        field.anim_traj(
            robot, traj, f"{alliance}_bottom_cube_cube_cube_score.gif", save_gif=False
        )
