...
def plan(alliance, quiet):
    ...
    write_to_csv(traj, f"{alliance}_top_cube")

    # Plot
    field.plot_traj(robot, traj, f"{alliance}_top_cube.png", save=True, quiet=quiet)

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, f"{alliance}_top_cube.gif", save_gif=False)
