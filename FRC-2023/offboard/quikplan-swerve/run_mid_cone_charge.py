...
def plan(alliance, quiet):
    ...
    write_to_csv(traj, f"{alliance}_mid_cone_charge")

    # Plot
    field.plot_traj(
        robot, traj, f"{alliance}_mid_cone_charge.png", save=True, quiet=quiet
    )

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, f"{alliance}_mid_cone_charge.gif", save_gif=False)
