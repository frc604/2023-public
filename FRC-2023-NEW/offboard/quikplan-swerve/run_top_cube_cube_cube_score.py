...
def plan(alliance, quiet):
    ...
    start_pose = pc.construct_pose(*TOP_START_POS_PC)
    qp = QuikPlan(
        field,
        robot,
        start_pose,
        [StoppedPoseConstraint(start_pose)],
        start_action=Action(ActionType.POOP_CHUTE, scoring_loc=(0, 1)),
    )

    # Leave community without turning
    waypoint1 = pc.construct_pose(*TOP_INTAKE_DEPLOY_PC)
    qp.add_waypoint(
        waypoint1,
        10,
    ...
    write_to_csv(traj, f"{alliance}_top_cube_cube_cube_score")

    # Plot
    field.plot_traj(
        robot,
        traj,
        f"{alliance}_top_cube_cube_cube_score.png",
        save=True,
        quiet=quiet,
    )
    ...
    if not quiet:
        # Animate
        field.anim_traj(
            robot, traj, f"{alliance}_top_cube_cube_cube_score.gif", save_gif=False
        )
