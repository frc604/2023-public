...
def plan(alliance, quiet):
    ...
    qp = QuikPlan(field, robot, start_pose, [StoppedPoseConstraint(start_pose)])
    ...
    waypoint4andahalf = pc.construct_pose(*BOTTOM_CHARGE_STATION_INSIDE)
    qp.add_waypoint(
    ...
    write_to_csv(traj, f"{alliance}_bottom_zigzag_test")

    # Plot
    field.plot_traj(
        robot,
        traj,
        f"{alliance}_bottom_zigzag_test.png",
        save=True,
        quiet=quiet,
    )
    ...
    if not quiet:
        # Animate
        field.anim_traj(
            robot, traj, f"{alliance}_bottom_zigzag_test.gif", save_gif=False
        )
