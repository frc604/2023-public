...
def plan(alliance, quiet):
    ...
    waypoint1 = pc.construct_pose(*TOP_COMMUNITY_ENTRANCE)
    qp.add_waypoint(
        waypoint1,
        10,
    ...
    waypoint10 = pc.construct_pose(*TOP_SECOND_SCORING_POS)
    qp.add_waypoint(
        waypoint10,
        10,
        intermediate_constraints=[
    ...
    write_to_csv(traj, f"{alliance}_top_cone")

    # Plot
    field.plot_traj(robot, traj, f"{alliance}_top_cone.png", save=True, quiet=quiet)

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, f"{alliance}_top_cone.gif", save_gif=False)
