...
def plan(alliance, quiet):
    ...
    # Back up barely
    waypoint1 = pc.construct_pose(*BOTTOM_BACKUP_SLIGHTLY)
    qp.add_waypoint(
        waypoint1,
        10,
    ...
    write_to_csv(traj, f"{alliance}_bottom_one_piece")

    # Plot
    field.plot_traj(
        robot, traj, f"{alliance}_bottom_one_piece.png", save=True, quiet=quiet
    )

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, f"{alliance}_bottom_one_piece.gif", save_gif=False)
