...
def plan(alliance, quiet):
    ...
    waypoint9 = pc.construct_pose(*TOP_CHARGE_STATION)
    qp.add_waypoint(
        waypoint9,
        10,
    ...
    write_to_csv(traj, f"{alliance}_top_cone_charge")

    # Plot
    field.plot_traj(
        robot, traj, f"{alliance}_top_cone_charge.png", save=True, quiet=quiet
    )

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, f"{alliance}_top_cone_charge.gif", save_gif=False)
