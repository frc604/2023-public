...
def plan(alliance, quiet):
    ...
    write_to_csv(traj, f"{alliance}_bottom_cone_cube_score")

    # Plot
    field.plot_traj(
        robot, traj, f"{alliance}_bottom_cone_cube_score.png", save=True, quiet=quiet
    )

    if not quiet:
        # Animate
        field.anim_traj(
            robot, traj, f"{alliance}_bottom_cone_cube_score.gif", save_gif=False
        )
