...
public class RobotContainer {
  ...
  private final QuixPigeon2 imu =
      new QuixPigeon2(
          ...
          QuixPigeon2.makeDefaultConfig()
              .setGyroTrimZ(Constants.IMU.gyroTrimZ)
              .setUpdateFrequency(Constants.Test.kSwerveControlFrequency));
  ...
  private final QuixVisionSim visionSim = new QuixVisionSim(cameras);

  // Subsystems
  private final ScoringSelector scoringSelector = new ScoringSelector(fieldViz);
  private final Swerve swerve = new Swerve(imu, cameras, visionSim, fieldViz);
